...
function fonts() {
    return es.merge(gulp.src([config.app + 'content/custom-icon/fonts/*.*', config.bower + 'bootstrap/fonts/*.*', config.bower + 'font-awesome/fonts/*.*', config.bower + 'simple-line-icons/fonts/*.*'])
...
function libs() {
    return gulp.src(config.app + 'app/modules/**/*.js', { dot: false })
        .pipe(plumber({errorHandler: handleErrors}))
        .pipe(changed(config.dist + 'app/modules'))
        .pipe(gulp.dest(config.dist + 'app/modules'));
}
...
function download() {
    return gulp.src(config.app + 'download/**/*', { dot: false })
        .pipe(plumber({errorHandler: handleErrors}))
        .pipe(changed(config.dist + 'download'))
        .pipe(gulp.dest(config.dist + 'download'));
}
